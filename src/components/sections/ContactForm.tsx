    ...
    <section id="contact" className="relative py-24 md:py-48 px-6 text-center overflow-hidden bg-background border-t border-border">
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <h2 className="font-display text-[clamp(3rem,12vw,10rem)] font-extrabold leading-[0.95] tracking-tighter uppercase mb-16 fade-up">
          Have an<br />
          <em className="italic text-accent not-italic">idea?</em>
        </h2>
        
        <div className="flex flex-col items-center gap-6">
          <a 
            href="mailto:samar@example.com" 
            className="group relative inline-flex items-center gap-3 px-8 md:px-12 py-4 md:py-6 bg-foreground text-background font-display text-base md:text-lg font-extrabold tracking-tight rounded-full overflow-hidden transition-all hover:scale-105 active:scale-95 shadow-[0_20px_40px_rgba(0,0,0,0.1)]"
          >
          ...
          <div className="flex flex-wrap justify-center gap-4 md:gap-8 mt-6">
            {["GitHub", "LinkedIn", "Twitter"].map((social) => (
              <Link 
                key={social} 
                href="#" 
                className="text-[10px] md:text-[14px] uppercase tracking-[0.2em] font-medium text-foreground/40 border border-border rounded-full px-6 md:px-8 py-2.5 md:py-3.5 hover:bg-foreground hover:text-background hover:border-foreground transition-all"
              >
                {social}
              </Link>
            ))}
