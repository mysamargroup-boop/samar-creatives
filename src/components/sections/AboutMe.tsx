...
export function AboutMe() {
  ...
  return (
    <section id="about" className="py-24 md:py-40 px-6 md:px-12 bg-background">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start text-left">
        <div className="w-full">
          <span className="text-[11px] uppercase tracking-[0.3em] text-foreground/40 block mb-10 fade-up font-bold">About Me</span>
          <h2 className="font-display text-[clamp(3rem,9vw,7rem)] font-extrabold leading-[1.0] tracking-tighter uppercase w-full">
        ...
        <div className="space-y-10 w-full">
          <p className="fade-up text-[clamp(1.1rem,2vw,1.4rem)] leading-relaxed text-foreground/60">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 pt-12 border-t border-border">
            {stats.map((stat, i) => (
              <div key={i} className="fade-up">
                <div className="font-display text-[clamp(3rem,6vw,5rem)] font-extrabold leading-none tracking-tighter flex items-start">
                  {stat.num}
                  {stat.num !== "∞" && <span className="text-[0.5em] text-accent ml-1">+</span>}
                </div>
                <div className="text-[10px] md:text-[12px] uppercase tracking-[0.2em] text-foreground/30 mt-4 font-bold">
                  {stat.label}
                </div>
              </div>
            ))}
